 ...
 //   signal to split sinus
  // lfoPattern(); // if ive is not open  //lfoPattern(); // if Live is not open
    // if you dob't use this, uncomment signal2 in addSignalLfoPattern()
     textSize (100);
          text ("addSignalOneAndTwoTer " + signal[3], -width-200, -height- 1400-600 );

                  print ("addSignalOneAndTwoTer " + keyMode + " " + measure );
...
//  splitTimeLfoQuater(); //USELESS with propagation Mode
 ...
 // addSignalLfoPatternQuater(); // devenu fonction ci dessous
  ...
  //*_  propagationMode();
     splitTimeScale(50.0); // withSignal2
   //++  propagation2way();

   propagation2wayWithSignal2();

  
 

  // addSignalOneAndTwoTerOriginal();
...
void propagation2wayWithSignal2() {   // le boule d'apres prends la position de la boue d'vant + PI/8
        oscillatorChanged=oscillatorChangingPropagation;
 //  if ( oscillatorChanged==true){ // A essayer
     phaseKeptAtChange[oscillatorChange]= map (signal[2], 0, 1, 0, TWO_PI);
  //    }
